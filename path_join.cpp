...
vector<vector<pair<int,int>>> vecpair;
vector<int> array_info(11,0);
...
void opt_order_join(string filename)
{
    ...
    while(fin)
    {
        ...
        int u = stoi(tokens[0]), v = stoi(tokens[1]), veo = floor(stoi(tokens[2]));
        if(u < v) vecpair[u].push_back({v,veo});
        else vecpair[v].push_back({u,veo});
...
void run_min_prefix(string filename)
{
	...
	if (!local_lab_filter) {
 	...
 	ofstream fout("array_info.txt");
	for(int i = 5; i < 10; i++)
	{
		cout<<90+i<<" "<<array_info[i]<<endl;
		fout<<90+i<<" "<<array_info[i]<<endl;
	}
	fout.close();
}
