...
extern vector<vector<pair<int,int>>> vecpair;
extern vector<int> array_info;
...
int found(int x, int y, vector<vector<pair<int,int>>> &vec);
...
inline int found(int x, int y, vector<vector<pair<int,int>>> &vec)
{
	if(x > y) swap(x,y);
	for(int i = 0; i < vec[x].size(); i++)
	{
		if(vec[x][i].first == y) return vec[x][i].second;
	}
	return -1;
}
...
inline void rud_dist( Priority* pri, int& ans, bool filter_only = false) {
	...
	if(veo != -1);
	{
		...
		if (edist != OVER_BOUND) {
			if(array_info[veo-90] < edist) array_info[veo-90] = edist;
			if(edist == 3 && veo == 99) cout<<"hey"<<endl;
			++ ans;
			if (!print_more & print_ans)
				outfile << pri->lg->name << " " << pri->rg->name << " " << edist << endl;
		}
